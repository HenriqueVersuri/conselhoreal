...
export const UserFormModal: React.FC<{
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (isNewUser && trimmedPassword.length < 6) {
                setLocalError('Defina uma senha com pelo menos 6 caracteres para o novo usuário.');
                setIsSaving(false);
                return;
            }
            if (!isNewUser && trimmedPassword.length > 0 && trimmedPassword.length < 6) {
                setLocalError('Ao atualizar a senha, utilize pelo menos 6 caracteres.');
                setIsSaving(false);
                return;
            }
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
            <div className="relative bg-[#1A1A1A] text-[#FFF8E1] w-full max-w-2xl p-8 rounded-lg border border-[#333] shadow-lg shadow-black/50" onClick={e => e.stopPropagation()}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div>
                        <label htmlFor="user-password" className="block text-sm uppercase tracking-wider mb-2">Senha {user ? '(opcional)' : '(obrigatória)'}</label>
                        <div className="relative">
                            <input
                                ...
                                className="w-full bg-[#0B0B0B] p-3 border border-[#333] focus:border-[#D4AF37] focus:outline-none"
                                placeholder={user ? 'Informe para alterar a senha' : 'Defina a senha do usuário'}
                            />
                            <button
                                type="button"
                                className="absolute inset-y-0 right-3 text-xs text-gray-400 hover:text-[#D4AF37]"
                        ...
                        <p className="text-xs text-gray-500 mt-1">A senha precisa ter pelo menos 6 caracteres.</p>
                    </div>
                    {errorMessage && (
                        <p className="text-[#C0161D] text-sm bg-[#C0161D]/10 border border-[#C0161D]/40 p-3 rounded-md">{errorMessage}</p>
                    )}
                    {localError && (
                        <p className="text-[#C0161D] text-sm bg-[#C0161D]/10 border border-[#C0161D]/40 p-3 rounded-md">{localError}</p>
                    )}
                    <div className="pt-4 flex justify-end gap-4">
                        <button type="button" onClick={onClose} className="px-6 py-2 uppercase tracking-widest border border-gray-600 text-gray-400">Cancelar</button>
                        <button
